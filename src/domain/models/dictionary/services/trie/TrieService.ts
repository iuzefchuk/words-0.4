...
export default class TrieService {
  ...
  static async createNodeTree(array: SerializedNode, yieldControl: () => Promise<void>): Promise<Node> {
    ...
    const root: Node = { children: Object.create(null), isFinal: isFinalFlag === 1 };
    ...
    while (stack.length > 0) {
      ...
      if (frame.index < frame.letters.length) {
        ...
        const child: Node = { children: Object.create(null), isFinal: childArray[0] === 1 };
        frame.node.children[frame.letters[frame.index] as Letter] = child;
        frame.index++;
