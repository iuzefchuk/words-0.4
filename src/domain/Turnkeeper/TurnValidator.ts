...
import { Result as ValidationResult } from '@/domain/Turnkeeper/types/local/validation.ts';
...
export default class TurnValidator {
  ...
  execute(initialPlacement: Placement): ValidationResult {
    return InitialPlacementValidator.execute({ initialPlacement, context: this.context });
  }
}
